...
def render_device_dashboard(autoencoder):
    ...
    stream_mode = st.session_state.get(f"stream_mode_{dev_id}", "Simulate Normal (CICIDS Benign)")
    
    if scan_active:
        if stream_mode == "Simulate Normal (CICIDS Benign)":
            current_features = provider.get_next_benign(normalize=True)
        elif stream_mode == "Simulate DDoS Attack":
            current_features = provider.get_next_ddos(normalize=True)
        elif stream_mode == "Simulate PortScan":
            current_features = provider.get_next_portscan(normalize=True)
        else:
            current_features = np.array(dev_baseline)
            
        val_pkt_size, val_iat, val_entropy, val_symmetry = current_features
        # Sync values for charts
        st.session_state[f"pkt_{dev_id}"] = val_pkt_size
        st.session_state[f"iat_{dev_id}"] = val_iat
        st.session_state[f"ent_{dev_id}"] = val_entropy
        st.session_state[f"sym_{dev_id}"] = val_symmetry
    else:
        # If not scanning, just use whatever is currently saved
        val_pkt_size = st.session_state.get(f"pkt_{dev_id}", dev_baseline[0])
        val_iat      = st.session_state.get(f"iat_{dev_id}", dev_baseline[1])
        val_entropy  = st.session_state.get(f"ent_{dev_id}", dev_baseline[2])
        val_symmetry = st.session_state.get(f"sym_{dev_id}", dev_baseline[3])
    ...
    if scan_active:
        new_pkt = {
            "Time": now_str,
            "Pkt Size": round(val_pkt_size * 1500.0, 1),
            "IAT":      round(val_iat * 0.1, 4),
            "Entropy":  round(val_entropy * 8.0, 2),
            "Symmetry": round(val_symmetry * 1.0, 2),
            "Status":   "Safe" if is_safe else "Alert",
        }
        import pandas as pd
        new_df = pd.DataFrame([new_pkt])
        if st.session_state.packet_history[dev_id].empty:
            st.session_state.packet_history[dev_id] = new_df
        else:
            # Avoid pandas deprecation warning with empty concat
            hist_df = st.session_state.packet_history[dev_id]
    ...
    with st.container():
        with glass_card(stress_extra[0], f"box-shadow:0 0 15px rgba(0,255,65,0.2);{stress_extra[1]}"):
            ...
            with col_pulse:
                st.markdown(
                    "<div style='text-align:center;color:#00ff41;font-weight:600;margin-bottom:5px;"
            ...
            if not threats:
                ...
                for alert in threats:
                    st.markdown(
                        ...
                        f'margin-bottom:8px;background:rgba(255,0,0,0.08);border-radius:4px;">'
...
def _render_sidebar(dev_id, device_info, dev_baseline, disabled):
    ...
    with st.sidebar:
        ...
        <div style="background:rgba(0,255,65,0.05);padding:15px;border-radius:8px;
            ...
            <small style="color:#00ff41;font-weight:bold;">[ DEVICE REGISTRY ]</small><br>
        ...
        st.divider()
        st.markdown("### 📡 ThreatVision Data Stream")
        
        st.selectbox(
            "Select Real Traffic Source",
            ["Simulate Normal (CICIDS Benign)", "Simulate DDoS Attack", "Simulate PortScan"],
            key=f"stream_mode_{dev_id}",
            disabled=disabled
        )
