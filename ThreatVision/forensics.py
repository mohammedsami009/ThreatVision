"""Automated Forensic Incident Report Generator for ThreatVision.
...
def _render_report_pdf(report: ForensicReportData, output_path: str) -> str:
    ...
    styles.add(
        ParagraphStyle(
            ...
            fontSize=22,
            spaceAfter=12,
            textColor=colors.HexColor("#00ff41"),  # ThreatVision neon green
    ...
    # Title block
    story.append(Paragraph("<b>THREATVISION</b> :: FORENSIC INCIDENT REPORT", styles["ReportTitle"]))
    story.append(Spacer(1, 10))

    # Top summary table
    info_table_data = [
        ["Device ID:", report.device_id, "Sector:", report.sector],
        ["Device Name:", report.device_name, "Timestamp (UTC):", _format_timestamp(report.timestamp)],
        ["Trust Score:", f"{report.trust_score:.1f}%", "Severity:", report.severity],
        ["Status:", "QUARANTINED" if report.severity in ["CRITICAL", "HIGH"] else "MONITORING", "Attack Pattern:", report.attack_pattern],
    ]
    ...
    info_table.setStyle(
        TableStyle(
            [
                ...
                ("BACKGROUND", (2, 0), (2, -1), colors.HexColor("#111111")),
                ("TEXTCOLOR", (0, 0), (0, -1), colors.HexColor("#00ff41")),
                ("TEXTCOLOR", (2, 0), (2, -1), colors.HexColor("#00ff41")),
    ...
    metrics_table = Table(metrics_table_data, colWidths=[220, 200])
    ...
    if report.top_anomalies:
        ...
        anomalies_table = Table(anomalies_data, colWidths=[220, 200])
        anomalies_table.setStyle(
            TableStyle(
                [
                    ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#111111")),
                    ("TEXTCOLOR", (0, 0), (-1, 0), colors.HexColor("#ff007f")), # Neon Red/Pink for anomalies
                    ("FONTNAME", (0, 0), (-1, -1), "Courier"),
                    ("FONTSIZE", (0, 0), (-1, -1), 9),
                    ("GRID", (0, 0), (-1, -1), 0.5, colors.grey),
                    ("PADDING", (0, 0), (-1, -1), 5),
    ...
    story.append(Paragraph("Behavioral Timeline (Last 5 Events)", styles["SectionHeader"]))
    if report.threat_log:
        for entry in report.threat_log[:5]:
            timestamp = entry.get("time", "?")
            msg = entry.get("msg", "(no message)")
            story.append(Paragraph(f"<font color='#00ff41'>[{timestamp}]</font> {msg}", styles["Body"]))
    else:
    ...
    snap_table = Table(snap_data, colWidths=[250, 250])
    snap_table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#111111")),
    ...
    story.append(Spacer(1, 10))
    story.append(Paragraph("Cryptographic Incident Signature", styles["SectionHeader"]))
    story.append(Paragraph(f"<font name='Courier'>{report.incident_signature}</font>", styles["Body"]))
...
def _get_cfg(key, default=None):
    ...
    try:
        ...
        # Pass 2: search inside nested sections e.g. [smtp], [threatvision], [forensics]
...
def send_forensic_report(
    ...
    subject = "🚨 ThreatVision Critical Security Alert"
    body = (
        f"Device: {device_name}\n"
        f"Severity: {severity}\n"
        f"Trust Score: {trust_score:.1f}%\n\n"
        "A critical anomaly has been detected by the ThreatVision digital twin engine. "
...
def generate_and_send_report(
    ...
    output_dir = output_dir or _get_cfg(
        ...
        _get_cfg("THREATVISION_FORENSICS_OUT", "./reports"),
    )
    ...
    recipient_email = recipient_email or _get_cfg("THREATVISION_ALERT_RECIPIENT")
    if recipient_email:
        ...
        LOGGER.warning(
            "No recipient email provided for forensic report (THREATVISION_ALERT_RECIPIENT unset). PDF generated at %s",
            pdf_path,
        )
