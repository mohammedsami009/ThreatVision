...
def _resolve_iface(iface: str) -> str:
    ...
    try:
        ...
        # Pass 2: find any interface with a valid active IPv4
        for w in win_ifaces:
            for ip in w.get("ips", []):
                # Simple check for a valid IPv4 that isn't localhost or empty
...
def _make_callback(mac: str) -> callable:
    ...
    def _callback(pkt) -> None:  # type: ignore[no-untyped-def]
        try:
            # Safely extract MAC addresses
            src_mac, dst_mac = "", ""
            if Ether in pkt:
                src_mac = _norm_mac(pkt[Ether].src)
                dst_mac = _norm_mac(pkt[Ether].dst)
            elif hasattr(pkt, "src") and hasattr(pkt, "dst"):
                src_mac = _norm_mac(str(pkt.src))
                dst_mac = _norm_mac(str(pkt.dst))

            direction = "in" if dst_mac == mac_norm else "out"

            # Fallback: if they entered a dummy MAC, just capture everything and pretend it's "out"
            ...
            print(f"[sniffer debug] packet callback error: {e}")
...
def start_sniffer(mac: str, iface: str) -> None:
    ...
    def _run() -> None:
        try:
            ...
            # Removed strict BPF filter `filter=f"ether host {mac_lower}"` so it
            # captures general interface traffic and keeps the demo alive even with a dummy MAC.
            sniff(
