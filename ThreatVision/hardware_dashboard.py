"""hardware_dashboard.py — Live hardware device dashboard for ThreatVision.
...
def _resolve_status(trust_score: float):
    if trust_score >= 80:
        return True, False, NEON_GREEN, "neon-healthy", f"<span style='color:{NEON_GREEN};'>● ONLINE</span>"
    if trust_score >= 50:
        return False, False, "#ffb300", "neon-warning", "<span style='color:#ffb300;'>● WARNING</span>"
    if trust_score < 20:
        return (
            False, True, NEON_RED, "neon-critical",
...
def _try_send_hw_forensic_report(mac, device_info, trust_score, mse, jsd, dev_baseline, current_features):
    last_sent = st.session_state.last_alert_sent.get(mac)
    now_utc   = datetime.datetime.utcnow()
    # 10 minute cooldown per device to prevent email spam
    ...
    try:
        generate_and_send_report(
            ...
            device_data={
                ...
                "packet_history": st.session_state[_HW_PKT_HIS][mac].to_dict("records") if not st.session_state[_HW_PKT_HIS][mac].empty else [],
                "threat_log": st.session_state[_HW_THREAT_LOG][mac],
            },
        )
        st.toast("Forensic email dispatched successfully!", icon="📧")
        st.session_state.last_alert_sent[mac] = now_utc
...
def _render_hw_sidebar(hw_id: str, device_info: dict, mac: str, iface: str) -> None:
    with st.sidebar:
        st.markdown(
            f"<h1 style='text-align:center;color:{NEON_BLUE} !important;'>🛡️ ThreatVision Control</h1>",
        ...
        # Device info card
        st.markdown(f"""
        <div style="background:rgba(0,255,65,0.05);padding:15px;border-radius:8px;
                    border-left:4px solid #00ff41;margin:16px 0;">
            <small style="color:#00ff41;font-weight:bold;">[ LIVE HARDWARE ]</small><br>
        ...
        # Live telemetry
        display_mac = st.session_state.get("hw_mac", mac)
        display_iface = st.session_state.get("hw_iface", iface)
        pkt_count = get_packet_count(display_mac)
        st.markdown(f"""
        <div style="background:rgba(0,255,65,0.05);padding:12px;border-radius:8px;
                    border:1px solid rgba(0,255,65,0.2);margin-bottom:12px;">
            <small style="color:{NEON_GREEN};font-weight:bold;">CAPTURE TELEMETRY</small><br>
            <span style="color:#aaa;font-size:0.85em;">MAC:</span>
            <span style="color:white;font-family:monospace;font-size:0.85em;"> {display_mac or '—'}</span><br>
            <span style="color:#aaa;font-size:0.85em;">Interface:</span>
            <span style="color:white;font-size:0.85em;"> {display_iface or '—'}</span><br>
...
def render_hardware_dashboard(autoencoder) -> None:
    ...
    mac          = st.session_state.get("hw_mac", device_info.get("mac", ""))
    iface        = st.session_state.get("hw_iface", device_info.get("iface", ""))
    ...
    if calibrating:
        start_t = st.session_state.get("hw_calib_start", time.time())
        elapsed = int(time.time() - start_t)
        remaining = max(0, CALIBRATION_WINDOW - elapsed)

        st.warning(
            f"⚙️ **Calibrating baseline… ({remaining}s remaining)**. Trust score is suppressed during this window. "
            f"Sampling live traffic...",
            icon="📡",
        )
    ...
    # ── Top header ────────────────────────────────────────────────────────────
    st.markdown("""
    <div style="background:rgba(0,255,65,0.05);border:1px solid rgba(0,255,65,0.3);
                border-radius:4px;padding:5px;margin-bottom:15px;box-shadow:0 0 10px rgba(0,255,65,0.1);">
        <marquee scrollamount="5" style="color:#00ff41;font-family:'Source Code Pro',monospace;font-size:14px;letter-spacing:2px;">
            // LIVE CAPTURE: ACTIVE &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    with glass_card():
        ...
        with calib_col:
            if st.button("📡 Calibrate Baseline", key="hw_calibrate_btn", width="stretch"):
                ...
                st.session_state["hw_calib_start"] = time.time()
    ...
    with glass_card():
        ...
        c2.metric("Memory Usage", f"{mem_info.percent}%", help=f"Total: {mem_info.total / (1024**3):.1f} GB")
        c3.metric("Disk Usage", f"{disk_info.percent}%", help=f"Free: {disk_info.free / (1024**3):.1f} GB")
    ...
    if st.session_state[_HW_PKT_HIS][mac].empty:
        st.session_state[_HW_PKT_HIS][mac] = new_df
    else:
        hist_df = st.session_state[_HW_PKT_HIS][mac]
        if not hist_df.empty and hist_df.isna().all(axis=None):
            st.session_state[_HW_PKT_HIS][mac] = new_df
        else:
            st.session_state[_HW_PKT_HIS][mac] = pd.concat(
                [new_df, hist_df], ignore_index=True,
            ).head(12)
    if not is_safe:
        ...
        # Fire off the email report asynchronously based on hardware data
        _try_send_hw_forensic_report(mac, device_info, trust_score, mse, jsd, dev_baseline, current_features)
    ...
    with col_gauge:
        with glass_card(card_class):
            ...
            st.markdown(
                ...
                "font-size:12px;color:#00ff41;text-shadow:0 0 5px #00ff41;margin-top:-10px;'>"
    ...
    ) if stress_alert else ("", "border:1px solid rgba(0,255,65,0.2);")
    ...
    with glass_card(stress_extra[0], f"box-shadow:0 0 15px rgba(0,255,65,0.2);{stress_extra[1]}"):
    ...
    if st.session_state[_HW_MATH_MODE]:
        with glass_card(extra_style="border:1px solid #00ff41;box-shadow:0 0 15px rgba(0,255,65,0.2);"):
            ...
            with col_math:
                ...
                st.markdown(
                    f"<div style='color:#00ff41;font-weight:bold;text-align:center;padding:10px;"
                    f"border:1px solid #00ff41;border-radius:8px;margin-top:10px;"
                    f"box-shadow:0 0 10px rgba(0,255,65,0.2);'>"
    ...
    st_autorefresh(interval=1500, key="hw_dashboard_refresh")
