"""
ThreatVision · AI-Driven Digital Twin Dashboard
==============================================
...
load_dotenv()  # Must load before importing internal modules that read env vars!
...
from registry import IOT_REGISTRY, SESSION_DEFAULTS, CENTER_LAT, CENTER_LON
...
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="ThreatVision Overview",
    page_icon="🛡️",
    layout="wide",
...
if not has_users():
    admin_email    = os.environ.get("THREATVISION_ADMIN_EMAIL")
    admin_password = os.environ.get("THREATVISION_ADMIN_PASSWORD")
...
# ---------------------------------------------------------------------------
# Vibex integration helper
# ---------------------------------------------------------------------------
@st.cache_resource
def start_vibex_dashboard():
    """Launch Vibex Dash app in a daemon thread and keep alive."""
    try:
        import sys
        import os
        import importlib.util

        vibex_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "vibex"))
        dashboard_path = os.path.join(vibex_root, "dashboard.py")

        if not os.path.exists(dashboard_path):
            return False, f"Cannot find vibex dashboard at {dashboard_path}"

        module_name = "vibex_dashboard"
        if module_name in sys.modules:
            vibex_dashboard = sys.modules[module_name]
        else:
            spec = importlib.util.spec_from_file_location(module_name, dashboard_path)
            if spec is None or spec.loader is None:
                return False, "Failed to load vibex dashboard module spec"
            vibex_dashboard = importlib.util.module_from_spec(spec)
            sys.modules[module_name] = vibex_dashboard
            spec.loader.exec_module(vibex_dashboard)

        if "vibex_thread" not in st.session_state or not st.session_state.vibex_thread.is_alive():
            t = threading.Thread(
                target=lambda: vibex_dashboard.app.run(debug=False, port=8050, use_reloader=False),
                daemon=True,
            )
            t.start()
            st.session_state.vibex_thread = t
            return True, "Vibex dashboard starting on http://localhost:8050"

        return True, "Vibex dashboard already running on http://localhost:8050"
...
def load_threat_engine():
    ...
    weights_path = "threatvision_autoencoder.pth"
    if os.path.exists(weights_path):
        ...
        print(f"[model] No trained weights found at {weights_path} — using random weights")
    model.eval()
    return model

autoencoder = load_threat_engine()
...
def render_fleet_page():
    ...
    st.markdown("<h1 style='text-align:center;color:white;'>🌐 ThreatVision Command Center</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align:center;color:#aaa;'>Click a hotspot on the map or a row in the registry to open its Digital Twin dashboard. <span style='color:#00ff41'>● Green = Healthy</span> | <span style='color:#ff0000'>● Red = Compromised</span></p>", unsafe_allow_html=True)

    # --- Ticker Banner ---
    ticker_text = "⬡ THREAT_VISION ACTIVE · 10 NODES MONITORED · GLOBAL SENSORS · LSTM AUTOENCODER: RUNNING · DIGITAL TWINS: SYNCHRONIZED · THREAT LEVEL: NOMINAL · ENCRYPTION: AES-256-GCM · UPTIME: 99.98% · ANOMALY DETECTION: ENABLED · RECONSTRUCTION ERROR: NOMINAL · JSD DIVERGENCE: 0.00 ⬡"
    st.markdown(f"""
        <div style="background: rgba(0,255,65,0.03); border-top: 1px solid rgba(0,255,65,0.3); border-bottom: 1px solid rgba(0,255,65,0.3); padding: 8px 0; margin: 15px 0; overflow: hidden; white-space: nowrap;">
            <div style="display: inline-block; white-space: nowrap; animation: ticker 40s linear infinite; font-family: 'Source Code Pro', monospace; color: #00ff41; font-size: 0.85rem; text-shadow: 0 0 5px rgba(0,255,65,0.5); letter-spacing: 1px;">
    ...
    # Map initialization centered on configurable coords
    ...
    for idx, (dev_id, info) in enumerate(IOT_REGISTRY.items()):
        ...
        <div style="font-family:'Inter',sans-serif; background:rgba(17,25,40,0.95); color:white; padding:12px; border:1px solid {color}; border-radius:8px; box-shadow:0 0 10px {color}66; min-width:180px;">
            ...
            <code style="color:#00ff41; font-size:0.85em;">{dev_id}</code>
    ...
    with col_reg:
        ...
        st.markdown(f"""
        <div style="background:rgba(8,14,28,0.85);backdrop-filter:blur(20px);border:1px solid rgba(0,255,65,0.15);
                    border-radius:12px;padding:20px 20px 10px 20px;margin-bottom:8px;">
            <div style="font-family:'Source Code Pro',monospace;color:#00ff41;font-size:1.1rem;
                        margin-bottom:12px;letter-spacing:1px;">// DEVICE REGISTRY</div>
            <div style="display:flex;gap:10px;margin-bottom:4px;">
                <div style="flex:1;background:rgba(0,255,65,0.08);border:1px solid rgba(0,255,65,0.3);
                            border-radius:8px;padding:8px 12px;text-align:center;">
                    <div style="color:#00ff41;font-size:1.4rem;font-weight:bold;">{total_devices}</div>
                    <div style="color:#aaa;font-size:0.75rem;">TOTAL</div>
                </div>
                <div style="flex:1;background:rgba(0,255,65,0.08);border:1px solid rgba(0,255,65,0.3);
                            border-radius:8px;padding:8px 12px;text-align:center;">
                    <div style="color:#00ff41;font-size:1.4rem;font-weight:bold;">{healthy_count}</div>
                    <div style="color:#aaa;font-size:0.75rem;">ACTIVE</div>
                </div>
                <div style="flex:1;background:rgba(255,0,0,0.08);border:1px solid rgba(255,0,0,0.3);
                            border-radius:8px;padding:8px 12px;text-align:center;">
                    <div style="color:#ff0000;font-size:1.4rem;font-weight:bold;">{compromised_count}</div>
        ...
        search_query = st.text_input(
            "Search Registry", placeholder="🔍 Search devices...",
            key="registry_search",
            ...
            for dev_id, info in IOT_REGISTRY.items():
                ...
                with cb:
                    if st.button("▶", key=f"reg_nav_{dev_id}", help=f"Open {info['name']}"):
                        ...
                        st.rerun()
                c1.markdown(f"<span style='color:#00ff41;font-family:monospace;font-size:0.8em;'>{dev_id}</span>", unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
# Navigation (ThreatVision + Vibex)
# ---------------------------------------------------------------------------
selected_app = st.sidebar.radio(
    "Application",
    ["ThreatVision", "Realtime ThreatVision"],
    index=0,
    help="Switch between ThreatVision and Vibex dashboards",
)

if selected_app == "Realtime ThreatVision":
    st.session_state.page = "vibex"
else:
    if st.session_state.get("page") == "vibex":
